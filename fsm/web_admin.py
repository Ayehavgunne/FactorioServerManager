...
from fsm.util import run_in_thread, render_template
from fsm.util import check_password
import fsm.templates.filters
...
class CustomWS(WebSocket):
    ...
    def opened(self):
        log.debug(
            f'{self.__class__.__name__.replace("WSHandler", "")} Socket was opened'
        )
    ...
    def closed(self, code, reason=None):
        self.broadcast = False
        log.debug(
            f'{self.__class__.__name__.replace("WSHandler", "")} Socket was closed'
        )
