...
from fsm import OS_WIN, app_settings
    ...
    def validate(self, document):
        ...
        if not executable.is_file():
            raise ValidationError(
                message="That path does not contian a Factorio executable"
            )
...
class PortNumberValidator(Validator):
    def validate(self, document):
        ...
        if error:
            raise ValidationError(
                message="That is not a valid port number. Please try again."
            )
...
class FactorioPortNumberValidator(PortNumberValidator):
    def validate(self, document):
        ...
        if port in app_settings.factorio_instances.values():
            raise ValidationError(
                message="That port number is already taken by another Factorio instance"
            )
...
class FactorioInstanceNameValidator(Validator):
    def validate(self, document):
        ...
        if name in app_settings.factorio_instances.keys():
            raise ValidationError(
                message="That Factorio instance name is already taken"
            )
...
class BooleanValidator(Validator):
    def validate(self, document):
        ...
        if text not in ("y", "n", "1", "0", "t", "f"):
            raise ValidationError(
                message="You must type y, 1, or t for yes or type n, 0, f for no"
            )
